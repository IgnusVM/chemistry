...
import { buildAssetWhere, ASSET_SEARCH_FIELDS, type AssetListParams } from "./where";
...
export default async function AssetsPage({
  ...
  const { q, department, status, type, location, condition, searchBy } = params;
  const page = parsePage(params.page);
  const pageSize = parsePageSize(params.pageSize);

  const where = await buildAssetWhere(params);

  const [assets, total, departments, assetTypes, locations] = await Promise.all([
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <ExportDialog
            ...
            filterParams={{ q, searchBy, department, status, type, location, condition }}
            total={total}
          />
